...
def _get_levels(adjacency_matrix):
    ...
    """
    n = adjacency_matrix.shape[0]
    levels = t.zeros((n,), dtype=t.int64)
...
def _adjacency2lca(adjacency_matrix):
    ...
    is_leaf = (adjacency_matrix.sum(axis=0) == 1).type(t.int64)
...
def adjacency2lca(adjacency_matrix):
    ...
    if not isinstance(adjacency_matrix, t.Tensor):
        try:
            adjacency_matrix = t.Tensor(adjacency_matrix)
