...
import os
sys.path.append("/Users/siweiliu/Desktop/BaumBauen-main/src")
import click
...
import baumbauen.config as config
import baumbauen.data as data
import baumbauen.models as models
import baumbauen.utils as utils
import baumbauen.losses as losses
import baumbauen.ignite as ignite
...
parser.add_argument('-c', '--cfg_path', default="config.yaml",
...
parser.add_argument('-m', '--model',
              choices=(['transformer_model', 'nri_model']),
              help='Model architecture to use')
parser.add_argument('-d', '--dataset',
              type=int, help='Individual dataset to load')
parser.add_argument('-v', '--verbose',  action="store_true",
              help='select logs with verbose levels')
parser.add_argument('-vv', '--very_verbose',  action="store_true",
              help='select logs with very-verbose levels')
...
def main():
    ...
    # Load configs
    configs, tags = config.load_config(
        Path(args.cfg_path).resolve(),
        model=args.model,
    ...
    # Load datasets
    mode_tags = data.create_dataloader_mode_tags(configs, tags)
    ...
    if selected_model == 'transformer_model':
        ...
        model = models.BBTransformerBaseline(
    ...
    if configs[selected_model]['loss'] == 'cross_entropy':
        ...
        loss_fn = losses.FocalLoss(gamma=2.5, ignore_index=-1, weight=class_weights)
    ...
    bb_ignite_trainer = ignite.BBIgniteTrainer(
        model=model,
        optimizer=optimizer,
        loss_fn=loss_fn,
        device=device,
    ...
    bb_ignite_trainer.setup_handlers(
        cfg_filename=Path(args.cfg_path).name,
    )
