...
class Efficiency(Metric, object):
    ...
    def __init__(
        self,
        ignore_index=-1.0,
        ignore_disconnected_leaves=False,
    ...
    def _is_valid_lca(self, y_pred, y):
        ...
        bare_y_pred = y_pred[get_rows][
            :, get_rows
        ]  # (d1, d2) padding rows/cols removed
    ...
    def update(self, output):
        ...
        assert winners.shape == y.shape, print(
            f" winners: {winners.shape}, y: {y.shape}"
        )  #
...
class Pad_Accuracy(Metric, object):
    ...
    def __init__(self, ignored_class, output_transform=lambda x: x, device="cpu"):
        self.ignored_class = (
            ignored_class if isinstance(ignored_class, list) else [ignored_class]
        )
        self.device = device
        self._num_correct = None
        self._num_examples = None
        super(Pad_Accuracy, self).__init__(
            output_transform=output_transform, device=device
        )
    ...
    def compute(self):
        if self._num_examples == 0:
            raise NotComputableError(
                "Pad_Accuracy must have at least one example before it can be computed."
            )
...
class PerfectLCA(Metric, object):
    ...
    def __init__(self, ignore_index=-1.0, output_transform=lambda x: x, device="cpu"):
