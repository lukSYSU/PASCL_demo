    ...
    for tag, path, mode in tags:
        if configs["dataset"]["source"] == "phasespace":
            dataset = PhasespaceSet(
                root=path,
                mode=mode,
                file_ids=configs["dataset"]["datasets"]
                if "datasets" in configs["dataset"]["phasespace"]
                else None,
            ...
            if (
                configs["dataset"]["phasespace"]["config"]["apply_scaling"]
                and configs["dataset"]["phasespace"]["config"]["scaling_dict"] is None
            ):
                configs["dataset"]["phasespace"]["config"][
                    "scaling_dict"
            ...
            raise NotImplementedError(
                f"{configs['dataset']['source']} data source not implemented."
            )
