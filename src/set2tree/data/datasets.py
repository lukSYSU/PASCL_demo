...
from ..utils import assign_parenthetical_weight_tuples, decay2lca
...
class TreeSet(torch.utils.data.Dataset):
    ...
    def __init__(self, x, y):
        ...
        self.lca = y
        self.edge_lists = {
            N: np.array(list(itertools.product(range(N), repeat=2)))
    ...
    def __getitem__(self, idx):
        leafs = self.leafs[idx]
        lca = self.lca[idx]
        num_nodes = leafs.shape[0]
        g = dgl.graph(
            (
                torch.LongTensor(self.edge_lists[num_nodes][:, 0]),
                torch.LongTensor(self.edge_lists[num_nodes][:, 1]),
            ),
            num_nodes=num_nodes,
        )
        g.ndata["leaf features"] = torch.FloatTensor(leafs)
        g = dgl.add_self_loop(g)
        g.edata["lca"] = torch.LongTensor(lca).flatten()
        return g


class PhasespaceSet(torch.utils.data.Dataset):
    def __init__(
        self,
        root,
        mode="train",
        ...
        **kwargs,
        ...
        if samples_per_topology is not None and samples_per_topology < min(
            self.samples_per_topology
        ):
            # Choose which samples to take from each file
            ids = [
                np.random.choice(i, samples_per_topology, replace=False)
                for i in self.samples_per_topology
            ]
    ...
    def _calculate_scaling_dict(self, n_samples=1000):
        ...
        x_sample = np.concatenate(
            [
                self.__getitem__(i).ndata["leaf features"]
                for i in np.random.choice(self.__len__(), size=n_samples, replace=False)
            ]
        )  # (n_samples*l, d)
    ...
    def __getitem__(self, idx):
        ...
        # g = dgl.add_self_loop(g)
        # print(g.edges())
        g.edata["lca"] = torch.LongTensor(item[1]).flatten()
        return g

    def __len__(self):
        # Handle case that we have selected samples randomly
        return (
            self.samples_per_topology.sum()
            if isinstance(self.samples_per_topology, np.ndarray)
            else len(self.x)
        )
    ...
    def collate_graphs(samples):
        batched_g = dgl.batch(samples, ndata=["leaf features"], edata=["lca"])
        return batched_g
